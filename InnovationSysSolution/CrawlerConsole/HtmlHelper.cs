using HN863Soft.ISS.CrawlerCommon;
using System;
...
namespace CrawlerConsole
{
    ...
    public sealed class HtmlHelper
    {
        private class BasicContent : IDisposable
        {
            ...
            /// HTML类型
            /// </summary>
            private static readonly ReadOnlyCollection<string> _HtmlMimeTypes = new ReadOnlyCollection<string>(new[]{"application/xhtml+xml","text/html",});

            /// <summary>
            /// XML类型
            /// </summary>
            private static readonly ReadOnlyCollection<string> _XmlMimeTypes = new ReadOnlyCollection<string>(new[]{"text/xml"});

            /// <summary>
            /// CSS类型
            /// </summary>
            private static readonly ReadOnlyCollection<string> _CssMimeTypes = new ReadOnlyCollection<string>(new[]{"text/css"});
            ...
            /// </summary>
            /// <returns> </returns>
            public virtual string GetTitle(string html)
            {
                return "";
            }
        ...
        private class HtmlContent : BasicContent
        {
            ...
            /// <returns></returns>
            public override string GetTitle(string html)
            {
                //默认标题
                ...
                //尝试1： 正文的标题一般在h1、h2、h3中，比title中的标题更干净
                string h1Filter = @"<h1.*?>.*?</h1>";
                string h2Filter = @"<h2.*?>.*?</h2>";
                string h3Filter = @"<h3.*?>.*?</h3>";
                ...
                if (match.Success)
                {
                    string h2 = Regex.Replace(match.Groups[0].Value, clearFilter, "");

                    if (!String.IsNullOrEmpty(h2) && title.StartsWith(h2))
                    {
                        return h2;
                    }
                }
                ...
                if (match.Success)
                {
                    string h3 = Regex.Replace(match.Groups[0].Value, clearFilter, "");

                    if (!String.IsNullOrEmpty(h3) && title.StartsWith(h3))
                    {
                        return h3;
                    }
                }
        ...
        private class HtmlProcessor : IProcessor
        {
                                            ...
                                            string attributeName, Uri referenceUri)
            {
                ...
                var urls = elements.Select(e => e.Attributes[attributeName]).Select(a => new Uri(referenceUri, System.Web.HttpUtility.HtmlDecode(a.Value)));
        ...
        public static HtmlHelper CreatHtml(Uri uri)
        {
            try
            {
                if (_processors == null)
                ...
                using (var response = (HttpWebResponse)request.GetResponse())
                {
                    ...
                    if (response.ContentType.Contains("text/html"))
                    {
                        ...
                        using (var crawlerContent = ProcessFactory.Create(uri, response))
                        {
                            ...
                            if (uri.ToString().Contains("http://www.bjeit.gov.cn/jxdt/tzgg/"))
                            {
                                string clearFilter = @"<.*?>";
                                string h4Filter = @"<h4.*?>.*?</h4>";
                                Match match = Regex.Match(html.SourceCode, h4Filter, RegexOptions.IgnoreCase);
                                if (match.Success)
                                {
                                    html.Title = Regex.Replace(match.Groups[0].Value, clearFilter, "");
                                }
                            }
                            else if (uri.ToString().Contains("http://www.hndrc.gov.cn/ar/"))
                            {
                                string clearFilter = @"<.*?>";
                                string titleFilter = @"<div id=cms_article_title class=title>.*?</div>";
                                Match match = Regex.Match(html.SourceCode, titleFilter, RegexOptions.IgnoreCase);
                                if (match.Success)
                                {
                                    html.Title = Regex.Replace(match.Groups[0].Value, clearFilter, "");
                                }
                            ...
                            {
                                html.Title = crawlerContent.GetTitle(crawlerContent.Text);
                            }
        ...
        //        {
        //            HtmlHelper html = CreatHtml(new Uri("http://www.baidu.com"));

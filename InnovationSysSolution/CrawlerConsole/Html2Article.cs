...
namespace CrawlerConsole
{
    public class Html2Article
    {
        ...
        public static ArticleDocument GetArticle(HtmlHelper html, string currentUrl, ref bool isGetContentSuccess)
        {
            try
            {
                if (html == null)
                ...
                if (html.SourceCode.Count(c => c == '\n') < 10)
                {
                    html.SourceCode.Replace(">", ">\n");
                }

                // 获取html，body标签内容
                string body = "";
                string bodyFilter = @"(?is)<body.*?</body>";
                Match m = Regex.Match(html.SourceCode, bodyFilter);
                if (m.Success)
                {
                    body = m.ToString();
                }
                ...
                string title2 = GetTitle(html.SourceCode);
                string title = GetMostProTitle(title1, title2, content);

                DateTime publishDate = GetPublishDate(html.SourceCode);
                ArticleDocument article = new ArticleDocument
                {
                    ...
                    Site = GetSiteName(html.Title),
                    ChildrenLink = html.ChildrenLink.Select(o => o.ToString()).ToList()
                };

                isGetContentSuccess = article.Content.Length > 0;
                return article;
            }

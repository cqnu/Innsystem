...
namespace HN863Soft.ISS.Common
{
    public class DataTableHelper
    {
        ...
        public static T DataRowToEntity<T>(IDataReader drData) where T : class
        {
            ...
            for (int i = 0; i < drData.FieldCount; i++)
            {
                if (drData[i] == null)
                    continue;
                System.Reflection.PropertyInfo currentProperty = typeof(T).GetProperty(drData.GetName(i));
                if (currentProperty == null) continue;
                if (currentProperty.PropertyType.IsEnum)
                    currentProperty.SetValue(entity, Enum.ToObject(currentProperty.PropertyType, drData[i]), null);
                else
                    currentProperty.SetValue(entity, drData[i], null);
            }
            return entity;
        }

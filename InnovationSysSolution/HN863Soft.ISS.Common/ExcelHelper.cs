...
namespace HN863Soft.ISS.Common
{
    public class ExcelHelper
    {
        ...
        public static void ExportExcel(string templateFilePath, string savefilePath, object obj, DataTable dt, int rowIndex, int colIndex)
        {
            //生成Excel程序对象
            ...
            Excel._Workbook workbook = workbooks.Add(templateFilePath); //加载模板
            ...
            try
            {
                if (obj != null)
                {
                    var bookMarks = app.Names;
                    foreach (Excel.Name book in bookMarks)
                    {
                        string ExcelMarkName = book.Name;//Excel模板中的书签名称
                        try
                        {
                            app.Goto(ExcelMarkName);
                            app.ActiveCell.FormulaR1C1 = WordHelper.GetObjPropertyValue(obj, book.Name);
                        }
                        catch { }
                    }
                }
                if (dt != null && dt.Rows.Count > 0)
                {
                    for (int j = 0; j < dt.Rows.Count; j++)
                    {
                        ...
                        int currentRow = rowIndex + j + 1;
                        worksheet.Cells[currentRow, colIndex + 1] = (j + 1).ToString();
                        for (int i = 0; i < dt.Columns.Count; i++)
                        {
                            if (string.IsNullOrWhiteSpace(dr[i].ToString()) == false)
                            {
                                worksheet.Cells[currentRow, colIndex + 1 + i] = dr[i].ToString();
                            }
                        }
                    }
                }

                workbook.Close(true, savefilePath, Type.Missing);
                workbooks.Close();
                app.Quit();
                workbook = null;
                workbooks = null;
    ...
    public class NPOIHelper
    {
        ...
        public static void ExportByWeb(DataTable dtSource, string strHeaderText, string strFileName)
        {
            ...
            curContext.Response.BinaryWrite(ExportDataTableToExcel(dtSource, strHeaderText).GetBuffer());
        ...
        /// <param name="dtSource">源DataTable</param>
        /// <param name="strHeaderText">表头文本</param>
        public static MemoryStream ExportDataTableToExcel(DataTable dtSource, string strHeaderText)
        {
            HSSFWorkbook workbook = new HSSFWorkbook();
            HSSFSheet sheet = workbook.CreateSheet() as HSSFSheet;

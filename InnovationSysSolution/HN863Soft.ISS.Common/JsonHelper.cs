...
namespace HN863Soft.ISS.Common
{
    ...
    {
        public static T Get<T>(T currObject) where T : class
        {
            var entity = System.Activator.CreateInstance<T>();

            PropertyInfo[] proArray = typeof(T).GetProperties();
            foreach (PropertyInfo current in proArray)
            {
                object currValue = current.GetValue(currObject, null);
                //if (current.PropertyType.IsClass == true)
                //    currValue = current.GetValue(currObject, null);
                current.SetValue(entity, currValue, null);
            }
            return currObject;
        }

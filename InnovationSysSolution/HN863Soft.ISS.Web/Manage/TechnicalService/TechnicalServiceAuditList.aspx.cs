...
{
    public partial class TechnicalServiceAuditList : ManagePage
    {
        #region 变量定义
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                if (!ChkManageLevel("ChannelTechnicalServiceAuditList", EnumsHelper.ActionEnum.View.ToString())) //检查权限
                {
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("technicalserviceAudit_page_size", "ISSPage"), out _pagesize))
            {
        ...
        public void BindData()
        {
            ...
            string pageUrl = Utils.CombUrlTxt("TechnicalServiceAuditList.aspx", "keywords={0}&page={1}", this.keywords, "__id__");
        ...
        {
            Response.Redirect(Utils.CombUrlTxt("TechnicalServiceAuditList.aspx", "keywords={0}", txtKeywords.Text));
        }
        ...
        {
            if (!ChkManageLevel("ChannelTechnicalServiceAuditList", EnumsHelper.ActionEnum.Delete.ToString())) //检查权限
            {
                ShowMsgHelper.ShowScript("showWarningMsg('您没有管理该页面的权限');");
                return;
            }
            ...
            ShowMsgHelper.ShowScript("location.href='/Manage/TechnicalService/TechnicalServiceAuditList.aspx';");
            BindData();
        }
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("technicalserviceAudit_page_size", "ISSPage", _pagesize.ToString(), 14400);
                }
            }
            Response.Redirect(Utils.CombUrlTxt("TechnicalServiceAuditList.aspx", "keywords={0}", this.keywords));
        }

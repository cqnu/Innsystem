...
// 文件名（File Name）：Intellectual_List.cs
// 作者（Author）：邹峰
// 功能（Function）：知识产权信息列表
// 创建日期（Create Date）：2017/03/14
//*****************************
namespace HN863Soft.ISS.Web.Manage.Intellectual
{
    public partial class Intellectual_List : ManagePage
    {
        #region 变量定义
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                if (!ChkManageLevel("ChannelIntellectualList", EnumsHelper.ActionEnum.View.ToString())) //检查权限
                {
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("intel_page_size", "ISSPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        public void BindData()
        {
            ...
            string pageUrl = Utils.CombUrlTxt("Intellectual_List.aspx", "ddlId={0}&keywords={1}&page={2}", this.ddl_id.ToString(), this.keywords, "__id__");
        ...
        {
            if (!ChkManageLevel("ChannelIntellectualList", EnumsHelper.ActionEnum.Delete.ToString())) //检查权限
            {
                ShowMsgHelper.ShowScript("showWarningMsg('您没有管理该页面的权限');");
                return;
            }
            ...
            ShowMsgHelper.ShowScript("location.href='/Manage/Intellectual/Intellectual_List.aspx';");
        }
        ...
        {
            Response.Redirect(Utils.CombUrlTxt("Intellectual_List.aspx", "ddlId={0}&keywords={1}", ddlType.SelectedValue, txtKeywords.Text));
        }
        ...
        {
            Response.Redirect(Utils.CombUrlTxt("Intellectual_List.aspx", "ddlId={0}&keywords={1}", this.ddl_id.ToString(), txtKeywords.Text));
        }
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("intel_page_size", "ISSPage", _pagesize.ToString(), 14400);
                }
            }
            Response.Redirect(Utils.CombUrlTxt("Intellectual_List.aspx", "ddlId={0}&keywords={1}", this.ddl_id.ToString(), txtKeywords.Text));
        }

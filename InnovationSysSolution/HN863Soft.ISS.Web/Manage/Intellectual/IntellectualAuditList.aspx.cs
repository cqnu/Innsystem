...
{
    public partial class IntellectualAuditList : ManagePage
    {
        #region 变量定义
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                if (!ChkManageLevel("ChannelIntellectualAuditList", EnumsHelper.ActionEnum.View.ToString())) //检查权限
                {
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("intel_list_page_size", "ISSPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        public void BindData()
        {
            ...
            string pageUrl = Utils.CombUrlTxt("IntellectualAuditList.aspx", "ddlId={0}&keywords={1}&page={2}", this.ddl_id.ToString(), this.keywords, "__id__");
        ...
        {
            if (!ChkManageLevel("ChannelIntellectualAuditList", EnumsHelper.ActionEnum.Audit.ToString())) //检查权限
            {
                ShowMsgHelper.ShowScript("showWarningMsg('您没有管理该页面的权限');");
                return;
            }
            ...
            for (int i = 0; i < rptList.Items.Count; i++)
            {
                ...
                if (cb.Checked)
                {
                    HN863Soft.ISS.Model.Intellectual model = new HN863Soft.ISS.BLL.IntellectualBll().GetModel(id);
                    if (model == null)
                    {
                    ...
                    {
                        AddManageLog(EnumsHelper.ActionEnum.Audit.ToString(), "审核知识产权成功"); //记录日志

                        ShowMsgHelper.ShowScript("location.href='/Manage/Intellectual/IntellectualAuditList.aspx';");
                    }
                    else
                    {
                        ShowMsgHelper.ShowScript("showWarningMsg('知识产权审核失败！');");
                        return;
                    }
                }
            }
        ...
        {
            if (!ChkManageLevel("ChannelIntellectualAuditList", EnumsHelper.ActionEnum.Delete.ToString())) //检查权限
            {
                ShowMsgHelper.ShowScript("showWarningMsg('您没有管理该页面的权限');");
                return;
            }
            ...
            AddManageLog(EnumsHelper.ActionEnum.Delete.ToString(), "删除知识产权"); //记录日志
        ...
        {
            Response.Redirect(Utils.CombUrlTxt("IntellectualAuditList.aspx", "ddlId={0}&keywords={1}", ddlType.SelectedValue, txtKeywords.Text));
        }
        ...
        protected void lbtnSearch_Click(object sender, EventArgs e)
        {
            ...
            Response.Redirect(Utils.CombUrlTxt("IntellectualAuditList.aspx", "ddlId={0}&keywords={1}", this.ddl_id.ToString(), txtKeywords.Text));
        }
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("intel_list_page_size", "ISSPage", _pagesize.ToString(), 14400);
                }
            }
            Response.Redirect(Utils.CombUrlTxt("IntellectualAuditList.aspx", "ddlId={0}&keywords={1}", this.ddl_id.ToString(), txtKeywords.Text));
        }
        ...
        protected void btnJurisdiction_Click(object sender, EventArgs e)
        {
            for (int i = 0; i < rptList.Items.Count; i++)
            {
                ...
                if (cb.Checked)
                {
                    ...
                    if (pBll.UpdateJurisdiction("Intellectual", id, int.Parse(hidState.Value)))
                    {

                        if (int.Parse(hidState.Value) == 2)
                        {

                            HN863Soft.ISS.Model.Intellectual umodel = bll.GetModel(id);
                            ...
                            integralModel.Projectname = "Intellectual";
                        ...
                        AddManageLog(EnumsHelper.ActionEnum.Edit.ToString(), "修改知识产权用户查看权限"); //记录日志

                        ShowMsgHelper.ShowScript("location.href='/Manage/Intellectual/IntellectualAuditList.aspx';");
                    }
                    else
                    {
                        ShowMsgHelper.ShowScript("showWarningMsg('知识产权权限修改失败！');");
                        return;
                    }
                }
            }

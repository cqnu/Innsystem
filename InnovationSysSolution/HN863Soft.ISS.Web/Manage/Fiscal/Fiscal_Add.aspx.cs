...
{
    public partial class Fiscal_Add : ManagePage
    {
        #region 变量
        ...
        {
            if (!ChkManageLevel("ChannelFiscalList", EnumsHelper.ActionEnum.Add.ToString())) //检查权限
            {
            ...
            if (bll.Add(model) > 0)
            {
                AddManageLog(EnumsHelper.ActionEnum.Add.ToString(), "添加财税服务"); //记录日志

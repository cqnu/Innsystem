...
{
    public partial class FiscalAuditList : ManagePage
    {
        #region 变量定义
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                if (!ChkManageLevel("ChannelFiscalAuditList", EnumsHelper.ActionEnum.View.ToString())) //检查权限
                {
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("fiscal_audit_page_size", "ISSPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        public void BindData()
        {
            ...
            string pageUrl = Utils.CombUrlTxt("FiscalAuditList.aspx", "ddlId={0}&keywords={1}&page={2}", this.ddl_id.ToString(), this.keywords, "__id__");
        ...
        {
            if (!ChkManageLevel("ChannelFiscalAuditList", EnumsHelper.ActionEnum.Audit.ToString())) //检查权限
            {
                ShowMsgHelper.ShowScript("showWarningMsg('您没有管理该页面的权限');");
                return;
            }
            ...
            for (int i = 0; i < rptList.Items.Count; i++)
            {
                ...
                if (cb.Checked)
                {
                    HN863Soft.ISS.Model.Fiscal model = new HN863Soft.ISS.BLL.FiscalBll().GetModel(id);
                    if (model == null)
                    {
                    ...
                    {
                        AddManageLog(EnumsHelper.ActionEnum.Audit.ToString(), "审核工商注册"); //记录日志

                        ShowMsgHelper.ShowScript("location.href='/Manage/Fiscal/FiscalAuditList.aspx';");
                    }
                    else
                    {
                        ShowMsgHelper.ShowScript("showWarningMsg('财税服务审核失败！');");
                        return;
                    }
                }
            }
        ...
        {
            if (!ChkManageLevel("ChannelFiscalAuditList", EnumsHelper.ActionEnum.Delete.ToString())) //检查权限
            {
                ShowMsgHelper.ShowScript("showWarningMsg('您没有管理该页面的权限');");
                return;
            }
        ...
        {
            Response.Redirect(Utils.CombUrlTxt("FiscalAuditList.aspx", "ddlId={0}&keywords={1}", ddlType.SelectedValue, txtKeywords.Text));
        }
        ...
        {
            Response.Redirect(Utils.CombUrlTxt("FiscalAuditList.aspx", "ddlId={0}&keywords={1}", this.ddl_id.ToString(), txtKeywords.Text));
        }
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("fiscal_audit_page_size", "ISSPage", _pagesize.ToString(), 14400);
                }
            }
            Response.Redirect(Utils.CombUrlTxt("FiscalAuditList.aspx", "ddlId={0}&keywords={1}", this.ddl_id.ToString(), txtKeywords.Text));
        }
        ...
        protected void btnJurisdiction_Click(object sender, EventArgs e)
        {
            for (int i = 0; i < rptList.Items.Count; i++)
            {
                ...
                if (cb.Checked)
                {
                    ...
                    if (pBll.UpdateJurisdiction("Fiscal", id, int.Parse(hidState.Value)))
                    {
                        if (int.Parse(hidState.Value) == 2)
                        {

                            HN863Soft.ISS.Model.Fiscal umodel = bll.GetModel(id);
                            ...
                            integralModel.Projectname = "Fiscal";
                        ...
                        AddManageLog(EnumsHelper.ActionEnum.Edit.ToString(), "修改财税服务用户查看权限"); //记录日志

                        ShowMsgHelper.ShowScript("location.href='/Manage/Fiscal/FiscalAuditList.aspx';");
                    }
                    else
                    {
                        ShowMsgHelper.ShowScript("showWarningMsg('财税服务权限修改失败！');");
                        return;
                    }
                }
            }

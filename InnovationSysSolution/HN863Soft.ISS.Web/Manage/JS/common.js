...
function checkNumber(e) {
    ...
    if ((48 <= keynum && keynum <= 57) || keynum == 8 || keynum == 46 || keynum == 37 || keynum == 39 || (keynum > 95 && keynum < 106)) {
        return true;
    } else {
        return false;
    }
...
//判断是否为审核 才能删除
function ExeDelete(objId, objmsg) {
    if ($(".checkall input:checked").size() < 1) {
        parent.dialog({
            title: '提示',
            content: '对不起，请选中您要操作的记录！',
...
//设置普通用户不可看
function ExeJurisdiction(objId, objmsg) {
    if ($(".checkall input:checked").size() < 1) {
        parent.dialog({
            title: '提示',
            content: '对不起，请选中您要操作的记录！',
    ...
    parent.dialog({
        ...
        content: ' <script> ' +
                 ...
                 ' } ' +
                  ' </' + 'script> ' + '  是：<input name="Fruit" type="radio" value="Yes"  onclick="to_change()" />&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 否:<input name="Fruit" type="radio" value="No" checked onclick="to_change()" /> </br>    ',
        okValue: '确定',
        widht: 500,
        heigth: 500,
        ok: function () {

...
{
    public partial class RoadshowAuditList : ManagePage
    {
        #region 变量定义
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                if (!ChkManageLevel("ChannelRoadshowAuditList", EnumsHelper.ActionEnum.View.ToString())) //检查权限
                {
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("roadshow_audit_page_size", "ISSPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        public void BindData()
        {
            ...
            string pageUrl = Utils.CombUrlTxt("RoadshowAuditList.aspx", "ddlId={0}&keywords={1}&page={2}", this.ddl_id.ToString(), this.keywords, "__id__");
        ...
        {
            Response.Redirect(Utils.CombUrlTxt("RoadshowAuditList.aspx", "ddlId={0}&keywords={1}", this.ddl_id.ToString(), txtKeywords.Text));
        }
        ...
        {
            if (!ChkManageLevel("ChannelRoadshowAuditList", EnumsHelper.ActionEnum.Delete.ToString())) //检查权限
            {
                ShowMsgHelper.ShowScript("showWarningMsg('您没有管理该页面的权限');");
                return;
            }
            ...
            for (int i = 0; i < rptList.Items.Count; i++)
            {
                ...
                if (cb.Checked)
                {
                    ...
                    if (state == 1)
                    {
                        //ShowScriptMsg("已审核通过无法删除！", "RoadshowAuditList.aspx", "parent.loadMenuTree");
                        //return;
            ...
            ShowMsgHelper.ShowScript("location.href='/Manage/Roadshow/RoadshowAuditList.aspx';");
        }
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("roadshow_audit_page_size", "ISSPage", _pagesize.ToString(), 14400);
                }
            }
            Response.Redirect(Utils.CombUrlTxt("RoadshowAuditList.aspx", "ddlId={0}&keywords={1}", this.ddl_id.ToString(), txtKeywords.Text));
        }
        ...
        {
            if (!ChkManageLevel("ChannelRoadshowAuditList", EnumsHelper.ActionEnum.Audit.ToString())) //检查权限
            {
                ShowMsgHelper.ShowScript("showWarningMsg('您没有管理该页面的权限');");
                return;
            }
            ...
            for (int i = 0; i < rptList.Items.Count; i++)
            {
                ...
                if (cb.Checked)
                {
                    HN863Soft.ISS.Model.Roadshow model = new HN863Soft.ISS.BLL.RoadshowBll().GetModel(id);
                    if (model == null)
                    {
                    ...
                    if (bll.Update(model))
                    {
                        AddManageLog(EnumsHelper.ActionEnum.Audit.ToString(), "审核路演成功"); //记录日志
        ...
        {
            Response.Redirect(Utils.CombUrlTxt("RoadshowAuditList.aspx", "ddlId={0}&keywords={1}", ddlType.SelectedValue, txtKeywords.Text));
        }
        ...
        protected void btnJurisdiction_Click(object sender, EventArgs e)
        {
            for (int i = 0; i < rptList.Items.Count; i++)
            {
                ...
                if (cb.Checked)
                {
                    ...
                    if (pBll.UpdateJurisdiction("RoadShow", id, int.Parse(hidState.Value)))
                    {

                        if (int.Parse(hidState.Value) == 2)
                        {

                            HN863Soft.ISS.Model.Roadshow umodel = bll.GetModel(id);
                            ...
                            integralModel.Projectname = "RoadShow";
                        ...
                        AddManageLog(EnumsHelper.ActionEnum.Edit.ToString(), "修改路演用户查看权限"); //记录日志

                        ShowMsgHelper.ShowScript("location.href='/Manage/Roadshow/RoadshowAuditList.aspx';");
                    }
                    else
                    {
                        ShowMsgHelper.ShowScript("showWarningMsg('路演权限修改失败！');");
                        return;
                    }
                }
            }

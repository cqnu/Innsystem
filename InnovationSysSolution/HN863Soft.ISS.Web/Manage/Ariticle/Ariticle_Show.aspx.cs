...
//*****************************
// 文件名（File Name）：Ariticle_Show.cs
// 作者（Author）：邹峰
// 功能（Function）：显示工业设计 可对主题进行回复。管理员或发布人可对评论进行删除
// 创建日期（Create Date）：2017/02/16
//*****************************
namespace HN863Soft.ISS.Web.userAriticle
{
    public partial class Show : ManagePage
    {
        #region 变量定义
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("ariticle_show_page_size", "ISSPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        private void ShowInfo()
        {
            ...
            //int BeID = 0;
            strWhere.Append("and a.Ariticleid =" + id);
            ...
            dt = bll.ShowAriticleClass(this.pageSize, this.page, strWhere.ToString(), out this.totalCount);

            DataSet dtL = new DataSet();
            dtL = bll.ShowAriticleClassInfo(strWhere.ToString());
            ...
            string pageUrl = Utils.CombUrlTxt("Ariticle_Show.aspx?id=" + ViewState["id"] + "&", "keywords={0}&page={1}", this.keywords, "__id__");
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("ariticle_show_page_size", "ISSPage", _pagesize.ToString(), 14400);
                }
            }
            Response.Redirect(Utils.CombUrlTxt("Ariticle_Show.aspx?id=" + ViewState["id"] + "&", "keywords={0}", this.keywords));
        }

        protected void ButTure_Click(object sender, EventArgs e)
        {
            HN863Soft.ISS.BLL.userAriticle bll = new HN863Soft.ISS.BLL.userAriticle();
            HN863Soft.ISS.Model.userAriticle model = new Model.userAriticle();

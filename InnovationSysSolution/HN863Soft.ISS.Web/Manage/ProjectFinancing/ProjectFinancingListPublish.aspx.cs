...
{
    public partial class ProjectFinancingListPublish : ManagePage
    {
        #region 变量定义
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                if (!ChkManageLevel("ChannelProjectFinancingPublishList", EnumsHelper.ActionEnum.View.ToString())) //检查权限
                {
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("project_financing_publish_page_size", "ISSPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        public void BindData()
        {
            ...
            string pageUrl = Utils.CombUrlTxt("ProjectFinancingListPublish.aspx", "ddlId={0}&keywords={1}&page={2}", this.ddl_id.ToString(), this.keywords, "__id__");
        ...
        {
            Response.Redirect(Utils.CombUrlTxt("ProjectFinancingListPublish.aspx", "ddlId={0}&keywords={1}", this.ddl_id.ToString(), txtKeywords.Text));
        }
        ...
        {
            if (!ChkManageLevel("ChannelProjectFinancingPublishList", EnumsHelper.ActionEnum.Delete.ToString())) //检查权限
            {
                ShowMsgHelper.ShowScript("showWarningMsg('您没有管理该页面的权限');");
                return;
            }
            ...
            for (int i = 0; i < rptList.Items.Count; i++)
            {
                ...
                if (cb.Checked)
                {
                    ...
                    if (state == 1)
                    {
                        //ShowScriptMsg("已审核通过无法删除！", "ProjectFinancingListPublish.aspx", "parent.loadMenuTree");
                        //return;
            ...
            ShowMsgHelper.ShowScript("location.href='/Manage/ProjectFinancing/ProjectFinancingListPublish.aspx';");
        }
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("project_financing_publish_page_size", "ISSPage", _pagesize.ToString(), 14400);
                }
            }
        ...
        {
            Response.Redirect(Utils.CombUrlTxt("ProjectFinancingListPublish.aspx", "ddlId={0}&keywords={1}", ddlType.SelectedValue, txtKeywords.Text));
        }
    }
}
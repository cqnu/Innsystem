...
using System;
using System.Web.UI;
// 文件名（File Name）：EnterpriseRegistration_Modify.cs
// 作者（Author）：邹峰
// 功能（Function）：编辑工商注册信息
// 创建日期（Create Date）：2017/03/14
//*****************************
namespace HN863Soft.ISS.Web.Manage.EnterpriseRegistration
{
    public partial class EnterpriseRegistration_Modify : ManagePage
    {
        #region 变量
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                if (!ChkManageLevel("ChannelEnterpriseRegList", EnumsHelper.ActionEnum.Edit.ToString())) //检查权限
                {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            if (bll.Update(model))
            {
                AddManageLog(EnumsHelper.ActionEnum.Edit.ToString(), "修改工商注册"); //记录日志

...
{
    public partial class EnterpriseRegistration_List : ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                if (!ChkManageLevel("ChannelEnterpriseRegList", EnumsHelper.ActionEnum.View.ToString())) //检查权限
                {
        ...
        {
            int _pagesize;
            if (int.TryParse(Utils.GetCookie("enterprise_reg_page_size", "ISSPage"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        public void BindData()
        {
            ...
            string pageUrl = Utils.CombUrlTxt("EnterpriseRegistration_List.aspx", "ddlId={0}&keywords={1}&page={2}", this.ddl_id.ToString(), this.keywords, "__id__");
        ...
        {
            if (!ChkManageLevel("ChannelEnterpriseRegList", EnumsHelper.ActionEnum.Delete.ToString())) //检查权限
            {
                ShowMsgHelper.ShowScript("showWarningMsg('您没有管理该页面的权限');");
                return;
            }
            ...
            ShowMsgHelper.ShowScript("location.href='/Manage/EnterpriseRegistration/EnterpriseRegistration_List.aspx';");
        }
        ...
        {
            Response.Redirect(Utils.CombUrlTxt("EnterpriseRegistration_List.aspx", "ddlId={0}&keywords={1}", ddlType.SelectedValue, txtKeywords.Text));
        }
        ...
        {
            Response.Redirect(Utils.CombUrlTxt("EnterpriseRegistration_List.aspx", "ddlId={0}&keywords={1}", this.ddl_id.ToString(), txtKeywords.Text));
        }
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Utils.WriteCookie("enterprise_reg_page_size", "ISSPage", _pagesize.ToString(), 14400);
                }
            }
            Response.Redirect(Utils.CombUrlTxt("EnterpriseRegistration_List.aspx", "ddlId={0}&keywords={1}", this.ddl_id.ToString(), txtKeywords.Text));
        }

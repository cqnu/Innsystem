...
namespace HN863Soft.ISS.DAL
{
	...
	public partial class SmsTemplate
	{
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(0) from SmsTemplate");
		...
		/// 增加一条数据
		/// </summary>
		public int Add(HN863Soft.ISS.Model.SmsTemplate model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into SmsTemplate(");
			strSql.Append("Title,CallIndex,Content,IsSys)");
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(HN863Soft.ISS.Model.SmsTemplate model)
		{
			StringBuilder strSql=new StringBuilder();
		...
		/// 得到一个对象实体
		/// </summary>
		public HN863Soft.ISS.Model.SmsTemplate GetModel(int ID)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ID,Title,CallIndex,Content,IsSys from SmsTemplate ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
			...
			HN863Soft.ISS.Model.SmsTemplate model=new HN863Soft.ISS.Model.SmsTemplate();
        ...
        /// 得到一个对象实体
        /// </summary>
        public Model.SmsTemplate GetModel(string callIndex)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select top 1 ID,Title,CallIndex,Content,IsSys from SmsTemplate");
            strSql.Append(" where CallIndex=@CallIndex");
		...
		/// 得到一个对象实体
		/// </summary>
		public HN863Soft.ISS.Model.SmsTemplate DataRowToModel(DataRow row)
		{
			HN863Soft.ISS.Model.SmsTemplate model=new HN863Soft.ISS.Model.SmsTemplate();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,Title,CallIndex,Content,IsSys ");
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" ID,Title,CallIndex,Content,IsSys ");
			strSql.Append(" FROM SmsTemplate ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select * FROM SmsTemplate");
            if (strWhere.Trim() != "")
            {
                strSql.Append(" where " + strWhere);
            }

...
namespace HN863Soft.ISS.DAL
{
    public class UrlRewrite
    {
        ...
        public bool Add(Model.UrlRewrite model)
        {
            try
            {
                string filePath = Utils.GetXmlMapPath(KeysHelper.FILE_URL_XML_CONFING);
                ...
                //创建子节点
                foreach (Model.UrlRewriteItem modelt in model.UrlRewriteItems)
                {
        ...
        {
            Model.UrlRewrite model = new Model.UrlRewrite();
            string filePath = Utils.GetXmlMapPath(KeysHelper.FILE_URL_XML_CONFING);
            XmlDocument doc = new XmlDocument();
            doc.Load(filePath);
            XmlNode xn = doc.SelectSingleNode("urls");
            ...
            if (xnList.Count > 0)
            {
                foreach (XmlElement xe in xnList)
                {
                    if (xe.Attributes["name"].Value.ToLower() == attrValue.ToLower())
                    {
                        ...
                        //再次遍历子节点
                        List<Model.UrlRewriteItem> lsItems = new List<Model.UrlRewriteItem>();
                        foreach (XmlElement xe1 in xe.ChildNodes)
                        {
                            if (xe1.NodeType != XmlNodeType.Comment && xe1.Name.ToLower() == "item")
                            {
                                Model.UrlRewriteItem item = new Model.UrlRewriteItem();
        ...
        public Hashtable GetList()
        {
            ...
            List<Model.UrlRewrite> ls = GetList("");
        ...
        {
            List<Model.UrlRewrite> ls = new List<Model.UrlRewrite>();
            string filePath = Utils.GetXmlMapPath(KeysHelper.FILE_URL_XML_CONFING);
            XmlDocument doc = new XmlDocument();
            doc.Load(filePath);
            XmlNode xn = doc.SelectSingleNode("urls");
                    ...
                    if (xe.Attributes["name"] != null)
                    {
                        if (!string.IsNullOrEmpty(channel))
                        {
                            if (xe.Attributes["channel"] != null && channel.ToLower() == xe.Attributes["channel"].Value.ToLower())
                            {
                                ...
                                model.UrlRewriteStr = Utils.DelLastChar(urlRewriteString.ToString(), "&");
                                model.UrlRewriteItems = lsItems;
                                ls.Add(model);
                            }
                        }

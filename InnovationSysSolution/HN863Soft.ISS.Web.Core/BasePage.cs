...
namespace HN863Soft.ISS.Web.Core
{
    ...
    {
        protected string WebsiteCssStyle
        {
            get;
            set;
        }
        ...
        protected internal Model.ChannelSite site = new Model.ChannelSite();
        /// <summary>
        /// 父类的构造函数
        /// </summary>
        public BasePage()
        ...
        /// 返回站点信息
        /// </summary>
        protected Model.ChannelSite GetSiteModel()
        {
            ...
            Model.ChannelSite modelt = SiteDomains.GetSiteDomains().SiteList.Find(p => p.BuildPath == sitePath);
            return modelt;
        }
        ...
        {
            Hashtable ht = new BLL.UrlRewrite().GetList(); //获得URL配置列表
            Model.UrlRewrite model = ht[_key] as Model.UrlRewrite; //查找指定的URL配置节点
        ...
        {
            //如果Session为Null
            if (HttpContext.Current.Session[KeysHelper.SESSION_USER_INFO] != null)
            {
                return true;
            }
            else
            {
                //检查Cookies
                string username = Utils.GetCookie(KeysHelper.COOKIE_USER_NAME_REMEMBER, "ISS");
                string password = Utils.GetCookie(KeysHelper.COOKIE_USER_PWD_REMEMBER, "ISS");
                if (username != "" && password != "")
                {
                    BLL.Users bll = new BLL.Users();
                    Model.Users model = bll.GetModel(username, password,  false);
                    if (model != null)
                    {
                        HttpContext.Current.Session[KeysHelper.SESSION_USER_INFO] = model;
                        return true;
                    }
                }
            }
        ...
        public Model.Users GetUserInfo()
        {
            if (IsUserLogin())
            {
                Model.Users model = HttpContext.Current.Session[KeysHelper.SESSION_USER_INFO] as Model.Users;
                if (model != null)
                {
        ...
        /// 判断用户是否已经登录(解决Session超时问题)
        /// </summary>
        public bool IsUserLoginF()
        {
            //如果Session为Null
            if (HttpContext.Current.Session[KeysHelper.ForegroundUser] != null)
            {
                return true;
            }
            else
            {
                //检查Cookies
                string username = Utils.GetCookie("username", "ISS");
                string password = Utils.GetCookie("password","ISS");
                if (username != "" && password != "")
                {
                    BLL.Users bll = new BLL.Users();
                    Model.Users model = bll.GetModel(username, password, false);
                    if (model != null)
                    {
                        HttpContext.Current.Session[KeysHelper.ForegroundUser] = model;
                        return true;
                    }
                }
            }
        ...
        /// 取得用户信息
        /// </summary>
        public Model.Users GetUserInfoF()
        {
            if (IsUserLoginF())
            {
                Model.Users model = HttpContext.Current.Session[KeysHelper.ForegroundUser] as Model.Users;
                if (model != null)
                {
                    //为了能查询到最新的用户信息，必须查询最新的用户资料
                    model = new BLL.Users().GetModel(model.ID);
        ...
        private string GetFirstPath(string requestPath)
        {
            ...
            if (requestPath.StartsWith(config.webpath + KeysHelper.DIRECTORY_REWRITE_ASPX + "/"))
            {
                indexNum = (config.webpath + KeysHelper.DIRECTORY_REWRITE_ASPX + "/").Length;
            }
        ...
        /// 参数个数是否匹配
        /// </summary>
        private bool IsUrlMatch(Model.UrlRewriteItem item, params object[] _params)
        {
            int strLength = 0;
